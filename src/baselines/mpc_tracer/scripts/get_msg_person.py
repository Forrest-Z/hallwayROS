...
from pedsim_msgs.msg import TrackedPersons, TrackedGroups

class Person_msg:
    def __init__(self):
        rospy.Subscriber("/persons", TrackedPersons, self.personCallback)
    ...
    def personCallback(self, data):
        ...
        #person_state_initial = [x,y,yaw,vx,xy]
        self.person_state_initial = [data.tracks[0].pose.pose.position.x,data.tracks[0].pose.pose.position.y
                                ,orientation_theta,data.tracks[0].twist.twist.linear.x,data.tracks[0].twist.twist.linear.y]
        # print(rot)
