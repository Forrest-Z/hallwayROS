...
DELTA_D = 0.1
DT = 0.5
APPROACHING_DIS = 1.0*DT*GOAL_SPEED*1.2  # goal distance
ARRIVED_DIS = 0.5
...
def calc_yaw(cx,cy,initial_state):
    cyaw = [0]*len(cx)
    # Set stop point
    for i in range(len(cx) - 1):  #0,1,2 ... ,len(cx) - 2
        if i==0:
            dx = cx[0]-initial_state[0]
            dy = cy[0]-initial_state[1]
        else:
            dx = cx[i + 1] - cx[i]
            dy = cy[i + 1] - cy[i]
        cyaw[i] = atan2(dy, dx)
...
def get_ReferencePath(cx, cy, initial_state):
    ...
    while True:
        ...
        index += 1
    cyaw = calc_yaw(cx,cy, initial_state)
...
class Duel_loop:
    ...
    def startCb(self, req):
        ...
        if mode == "controlling":
            cx, cy, cyaw = get_ReferencePath(self.pre_obj.refer_x,self.pre_obj.refer_y,initial_state)
